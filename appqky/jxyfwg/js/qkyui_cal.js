        ...
        "calendar": function (options) {
            ...
            return this.each(function (i) {  //这里的this 就是 jQuery对象。这里return 为了支持链式调用
				...
				if(isNull($this.find(".caldate").val())=="kong"){
					...
					change_value(Number(ttt[0]),Number(ttt[1]),Number(ttt[2]),tttt[0],tttt[1],$(create_id).find(".showdate"),$this.find(".caldate"));
					//渲染日期列表
					cal_Xr(Number(ttt[0]),Number(ttt[1]),Number(ttt[2]),$(create_id).find(".qky_cal_tab"));
					//初始化下拉默认值
					console.log(tttt[0],tttt[1]);
					$(create_id).find(".cal_year").html(ttt[0]);
					$(create_id).find(".cal_moon").html(ttt[1]);
					$(create_id).find(".cal_hour").html(tttt[0]);
					$(create_id).find(".cal_minute").html(tttt[1]);
				}
