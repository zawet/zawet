...
/***************************************************************************** 弹窗**************/
function alert_msg(tit,txt,type){
	$("#del_msg .qky_masktit span").html(tit);
	$("#del_msg .sigtext span").html(txt);
	if(type==1){$("#del_msg .qx,#del_msg .sigtext i").removeClass("yc");$("#del_msg .re").addClass("yc")}
	if(type==2){$("#del_msg .qxx").removeClass("yc");$("#del_msg .qx").eq(1).removeClass("yc");$("#del_msg .re").addClass("yc")}
	popIn('#del_msg');
}

function close_msg(){
	$("#del_msg .qky_masktit span").html("");
	$("#del_msg .sigtext span").html("");
	$("#del_msg .qx,#del_msg .sigtext i,#del_msg .qxx").addClass("yc");
	$("#del_msg .re").removeClass("yc");
	popOut('#del_msg');
}

//独立一弹层可容纳多个对话框
var dtid=0;
function alert_msg_dl(tit,txt,type){	
	$("#dl_maskmian .qky_masktit span").html(tit);
	$("#dl_maskmian .sigtext span").html(txt);
	dtid++;var isid="delmain"+dtid;
	$("#dl_maskmian .qky_maskmian").attr("id",isid).addClass("dp");
	$("#dl_maskmian .maskclose,#dl_maskmian .re").attr("onClick","close_msg_dl('#"+isid+"')");
	$("#dlmask .qky_maskbg").after($("#dl_maskmian").html());
	if(type==1){$("#"+isid+" .qx").removeClass("yc");$("#"+isid+" .re").addClass("yc")}
	popIn("#dlmask");
}

function close_msg_dl(id){
	$(id).removeClass("dp");
	var dpn=$("#dlmask .qky_maskmian.dp").length;
	$(id+" .qky_masktit span").html("");
	$(id+" .sigtext span").html("");
	$(id+" .qx").addClass("yc");
	$(id+" .re").removeClass("yc");
	popOut(id);
	if(dpn<=0){popOut("#dlmask");}
}
