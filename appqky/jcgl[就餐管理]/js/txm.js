...
//选择类型进行渲染
function isshow(){
	if(getchbox("#f_or_b")=="1"){Xrprint(mould_data,stu_data,0);$("#print_tit").html("打印预览（正反面）");}
	if(getchbox("#stu_info")=="1"){Xrprint(mould_data,stu_data,1);$("#print_tit").html("打印预览（单学生信息）");}
	if(getchbox("#only_f")=="1"){Xrprint(mould_data,stu_data,2);$("#print_tit").html("打印预览（只正面）");}
	if(getchbox("#only_b")=="1"){Xrprint(mould_data,stu_data,4);$("#print_tit").html("打印预览（只反面）");}
}
 
//根据数据和选择的类型进行渲染打印预览 mdata【模板数据妹子】,data【数据妹子】,type【姿势】（高级，难于理解就不要理解了，就算我自己写的，第二次再看，我也看不懂了！）
function Xrprint(mdata,data,type){
	/*****格式化数据阶段 （前戏，捣鼓捣鼓数据妹子）*****************************************************************************/
	var f_b=f=b=o_f="";
	var inhtml="";
	...
	var tmbox_w=$(".printview_box .tmbox .bg").width();//一个卡片多宽
	var tmbox_h=$(".printview_box .tmbox .bg").height()+4;//一个卡片多高
		...
		for(var jj=1;jj<=page;jj++){//循环页数
			  ...
			  for(var oo=0;oo<oneall;oo++){//循环一页内的个数
					...
					if(tmeq>=tmbox_count){//下标超过总数时不进行html的增加
						...
						tmphtml+='<div class="tmbox">'+$(".printview_box").eq(0).find(".tmbox").eq(tmeq).html()+'</div>';//把起初0里的html塞到缓存里
					}
			  }
...
function tmPrint(id){
   ...
   var dd=id.replace(".","");
   $(id).each(function(i) {
    ptinthtml+="<div class='"+dd+"' style='height:auto'>"+$(this).html()+"</div>";
});
